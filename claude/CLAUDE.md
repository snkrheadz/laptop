...
## 学習モード（Explanatory Mode）

新しいコードベースや技術を理解する際の活用方法。

### いつ使うか

- 初めて触るプロジェクトに参加した時
- 複雑な既存コードを理解したい時
- 新しいライブラリ/フレームワークを学ぶ時
- レガシーコードのリファクタリング前
...
```
「このコードベースの全体構造を説明して」
「このファイルが何をしているか、初心者にもわかるように解説して」
「この関数の処理フローをステップバイステップで」
「なぜこの設計になっているのか、背景を推測して」
```
...
| 目的 | 質問例 |
|------|--------|
| 全体把握 | 「このプロジェクトのアーキテクチャを図で説明して」 |
| 詳細理解 | 「このクラスの責務と依存関係を説明して」 |
| 設計意図 | 「なぜここでStrategyパターンを使っているのか」 |
| 影響範囲 | 「この関数を変更したら何に影響する？」 |

### 学習の記録

理解した内容は学習記録セクションにメモを残す:

```markdown
- [2025-01-15] src/auth/ - JWTベースの認証、リフレッシュトークンはRedisに保存
- [2025-01-15] src/api/middleware - 認証→レート制限→ログの順で適用
```
