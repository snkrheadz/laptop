---
description: Break an approved design.md into an ordered, independently-verifiable tasks.md (the implementation contract)
...
> 実装の準備完了: `/implement-with-notes specs/<id>/tasks.md` で実装し、`/eng:test-and-fix` で緑を確認、`/spec-review <id>` でレビュー、`/eng:create-pr` で PR を作成してください。並列でやるなら案件ごとに worktree を分けてください。
